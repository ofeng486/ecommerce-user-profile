...
├── docs/                 # 7 篇中文技术文档 + SQL Schema（2026-08-21 已清理过时/规划文档并统一中文命名）
...
| ClusterAnalysisController | `/admin/cluster-analysis` | 簇概览、簇内用户、重算聚类（Admin 用于重算、User 用于查看结果） |
...
├── router/         # staticRoutes(7) + adminRoutes(10) + userRoutes(16)，hash 模式
├── store/          # 唯一 Pinia store：user（isLogin/info/accessToken，localStorage 持久化）
├── views/          # 33 个页面组件（admin 7 + user 8 + 认证 3 + 异常 3 + landing 等）
...
**双角色页面体系**：
- **Admin 管理端**（`views/admin/index.vue`，玻璃拟态侧边栏，10 个路由）：运营数据总览、任务管理、数据生成、数据导入、系统用户、标签体系管理、聚类重算、个人中心、通知。管理端定位为「数据生产者 + 系统管理者」
- **User 门户**（`views/user/index.vue`，SaaS 左导航，16 个路由）：工作台、画像概览/列表/详情/标签分析、商品分析/用户聚类/复购与留存/流失预警、人群圈选/画像对比/人群包、AI 分析、通知。画像查看与业务分析归属用户门户
...
SecurityConfig：/auth/login、/register、/public/**、/system/health 公开；
               画像查看与业务分析（profiles 子集、product/repeat/churn/audience）仅 ROLE_USER；
               聚类接口同时开放 ADMIN(聚类重算)/USER(结果查看)；其余 /admin/** 仅 ADMIN；
...
- **interfaces**（2）：AuthController、UserProfileController MockMvc 集成
...
| # | 问题 | 状态 |
| --- | --- | --- |
| 1 | ~~标签定义管理缺前端页面~~：已补齐 `/system/tag-definition` 管理界面（后端 CRUD + 前端标签体系管理页） | 已解决 |
...
| 10 | ~~过时 / 规划文档~~（admin-console-analysis、project-analysis、business-planning-analysis、requirements-specification、architecture-diagram、landing-preview）**2026-08-21 已删除**，保留文档统一改为中文命名 | 已清理 |
...
1. **补齐标签定义管理前端页面**（已就绪：`/system/tag-definition` 标签体系管理页，下一步可按需扩展标签规则的可视化配置）；
2. **给 4 个分析 Service + 对比分析补测试**，用 H2 + 样例数据，防止 SQL 回归；
3. ~~核对 docs/database-model.md 与 data-dictionary.md~~ **已完成**（2026-08-21 对照最新 22 表结构更新，并统一 docs 中文命名与清理过时文档）；
