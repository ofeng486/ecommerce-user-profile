...
### 2.3 `sys_notification`

站内通知（任务完成/失败、数据、系统消息等），由后端异步写入。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | `BIGINT UNSIGNED` | 主键 |
| `user_id` | `BIGINT UNSIGNED` | 接收用户（`sys_user.id`） |
| `type` | `VARCHAR(20)` | 通知类型：`TASK`/`DATA`/`SYSTEM`/`AI` |
| `title` / `content` | `VARCHAR` | 标题与内容 |
| `is_read` | `TINYINT(1)` | 0 未读，1 已读 |
| `ref_type` / `ref_id` | -- | 关联业务类型与 ID（可跳转） |
...
K-Means 用户聚类结果（由 `run_local_pipeline.py` 写入，表定义在 `mysql-schema-migration.sql`）：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `user_id` | `BIGINT UNSIGNED` | 用户主键（PK） |
| `cluster_id` | `INT` | 聚类簇编号 |
| `data_version` | `VARCHAR(32)` | 分析批次版本 |
...
AI 智能分析问答历史（DeepSeek/Mock 回答持久化，ASTable 回看）：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | `BIGINT UNSIGNED` | 主键 |
| `user_id` | `BIGINT UNSIGNED` | 提问用户（`sys_user.id`） |
| `question` | `VARCHAR(500)` | 用户提问 |
| `answer` | `TEXT` | AI 回答文本 |
| `data_json` | `TEXT` | SQL 查询结果 JSON 数组（供表格/图表回看） |
| `created_at` | `DATETIME(3)` | 创建时间 |
...
### 10.3 `audience_package_users`

人群包圈选结果明细（指定用户与人群包的关联），由执行圈选落库：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | `BIGINT UNSIGNED` | 主键 |
| `package_id` | `BIGINT UNSIGNED` | 关联 `audience_package.id` |
| `user_id` | `BIGINT UNSIGNED` | 关联 `ecommerce_user.id` |
| `created_at` | `TIMESTAMP` | 加入时间 |
| UNIQUE | `(package_id, user_id)` | 同一用户在同一人群包中唯一 |

## 11. 画像对比分析

### 11.1 `comparison_task`
