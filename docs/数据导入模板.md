...
- **数据生成功能**：管理端 → 数据生成，调用 `bigdata-scripts/generate_data.py` 一键生成 7 表合规数据并自动导入（页面提供多档预设规模，也可自定义用户/商品/行为/订单参数）；
- **数据导入能力**：导入器实际支持 **10 类 importer 策略**（标准 7 表 + 复合总表 + 天池行为集/发票集），详见 `backend` 的 `infrastructure/importer` 包；标准 CSV 模板仍以上述 7 张表为准。
