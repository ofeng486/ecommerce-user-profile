...
数据库用于保存系统管理数据、电商基础业务模拟数据、Spark 分析任务状态以及用户画像结果。原始海量行为数据由 Python 生成脚本产出并直接由 PySpark 本地管线处理，MySQL 承担业务数据存储、后台管理、结果查询和可视化接口的数据支撑。
...
- 电商用户、商品、行为和订单数据由 Python 生成并导入 MySQL，作为画像计算的业务数据源。
- PySpark 本地管线从 MySQL 业务表计算画像，最终将面向接口查询的结果覆盖写回 MySQL 画像结果表。
- MySQL 保存当前或近期可视化查询结果。
