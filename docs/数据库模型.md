...
| 领域 | 数据表 | 主要用途 |
| --- | --- | --- |
| 系统管理 | `sys_user`、`sys_login_log`、`sys_notification` | 后台账号、User/Admin 权限、登录审计、站内通知 |
...
| 画像分析 | `profile_tag_definition`、`user_profile_tag`、`user_profile_summary`、`user_segment`、`ads_user_rfm`、`user_cluster` | 标签体系、用户画像、RFM 分层与 8 分类、K-Means 聚类结果 |
| 任务管理 | `spark_analysis_task` | Spark 分析任务状态 |
| 人群运营 | `audience_package`、`audience_rule`、`audience_package_users`、`comparison_task`、`comparison_result` | 智能人群包圈选、动态条件与明细、画像对比分析 |
| AI 辅助 | `ai_chat_history` | AI 智能分析问答历史 |
