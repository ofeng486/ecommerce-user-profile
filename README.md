...
基于 **Spring Boot + Spark** 的电商平台用户画像分析系统。
...
系统采用 **Admin 管理端** 与 **User 用户门户** 双角色，各有侧重：

### 管理端（Admin）——「数据生产者 + 系统管理者」

- **运营数据总览**：运营大盘（总用户 / 已画像用户 / 高价值用户 / 累计消费金额等总体指标）
- **数据生产**：任务管理、数据生成、数据导入、聚类重算
- **系统管理**：系统用户、标签体系管理
- **个人中心、通知**

### 用户门户（User）——「运营分析员 / 数据消费者」

- **工作台**：近期运营动态与入口
- **画像**：画像概览、画像列表、画像详情、标签分析
- **业务分析**：商品分析、用户聚类、复购与留存、流失预警
- **人群运营**：人群圈选、画像对比、人群包
- **AI 分析**：对话式智能分析
- **通知**

> 画像查看与业务分析（商品 / 聚类 / 复购 / 流失 / 人群）均归属**用户门户**；管理端仅承担数据生产与系统管理职能。

### 可视化

仪表盘、画像概览总览、RFM 分层分布、标签分析等（ECharts）。
...
系统没有预置默认账号。**首次运行通过注册页创建一个账号**，注册得到的均为**普通用户（User）**，直接就能使用用户门户（工作台、画像概览/列表/标签分析、商品/聚类/复购/流失分析、人群圈选、AI 分析等相关功能）。

要体验**管理端**，把该账号提升为管理员——在数据库中执行一行 SQL：

```sql
UPDATE sys_user SET role = 'Admin' WHERE username = '你的用户名';
```

> 权限边界：**画像查看与业务分析**（画像列表/详情、商品 / 复购 / 流失 / 人群圈选 / 画像对比 / 人群包）仅普通用户（User 门户）可用，后端强制 `ROLE_USER`；**数据生产与系统管理**（任务管理、数据生成、数据导入、聚类重算、系统用户、标签体系管理）仅管理员可用，后端强制 `ROLE_ADMIN`。管理端不再提供画像列表与业务分析入口。
