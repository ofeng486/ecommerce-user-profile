...
- **系统管理**：系统用户、标签体系管理
- **业务分析**（与 User 共用）：商品分析、复购与留存、流失预警、用户聚类、人群圈选、画像对比、人群包
- **个人中心、通知**

### 用户门户（User）——「运营分析员 / 数据消费者」

- **工作台**：近期运营动态与入口
- **画像**：画像概览、画像列表、标签分析、画像详情
- **业务分析**（与管理端共用页面）：商品分析、用户聚类、复购与留存、流失预警
- **人群运营**：人群圈选、画像对比、人群包
- **AI 分析**：对话式智能分析
- **通知**
...
> 权限边界：**业务分析与人群运营**（商品 / 复购 / 流失 / 聚类 / 人群圈选 / 画像对比 / 人群包）两端角色皆可用（后端 `ROLE_ADMIN` 与 `ROLE_USER` 均开放）；**数据生产与系统管理**（任务管理、数据生成、数据导入、聚类重算、系统用户、标签体系管理）仅管理员可用（后端强制 `ROLE_ADMIN`）。
