...
public class SalesOrderImporter extends AbstractCsvImporter {
    /** 业务唯一键（重复数据识别用） */
    protected String uniqueKeyColumn() { return "order_no"; }
    /** 同批导入时需同步映射的外键引用 */
    ...
    protected void validateRow(Map<String, String> row) {
        ...
        // 优惠金额留空按 0 处理（模板已精简该列）
        String discountRaw = get(row, "discount_amount");
        BigDecimal discount = discountRaw.isBlank() ? BigDecimal.ZERO : new BigDecimal(discountRaw);
